...
static int
find_input_selector (GstElement * element, gchar *type)
{
...
find_input_selector_with_type (GstBin * bin, const gchar *type)
{
  ...
  GstElement *input_selector = gst_iterator_find_custom (iterator,
      (GCompareFunc) find_input_selector, (gpointer) type);
...
find_sink_pad_index (GstElement * element, GstPad * pad)
{
  ...
  iterator = gst_element_iterate_sink_pads (element);
  while (!done) {
    switch (gst_iterator_next (iterator, (gpointer *) & tmppad)) {
      case GST_ITERATOR_OK:
        if (pad == tmppad) {
          done = TRUE;
        } else {
